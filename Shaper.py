...
def smoothen(obj,target):
    vg = obj.vertex_groups.new("Shaper_Smoothed")
    verts=[]
    bm=bmesh.new()
...
def shapetouv():
    ...
    if len(selection)==2:
        ...
        if(len(obj_active.data.uv_textures)==0):
            obj_active.data.uv_textures.new()
...
def uvtoshape():
    if bpy.context.scene.polycounter==0:
        ...
        if len(bpy.context.selected_objects)==2:
            ...
            for s in bpy.context.selected_objects: 
                if not s.name==bpy.context.active_object.name:
                    break
                obj_selected_index=obj_selected_index+1           
            bpy.context.scene.selected_name=bpy.context.selected_objects[obj_selected_index].name
            bpy.context.scene.active_name=bpy.context.active_object.name
            if bpy.context.selected_objects[obj_selected_index].data.uv_layers.active and bpy.context.active_object.data.uv_layers.active:
    ...
    if obj_selected.type=='MESH' and obj_active.type=='MESH' and bpy.context.scene.goflag and bpy.context.active_object.mode=='OBJECT':
...
def register():  
    bpy.utils.register_class(uvtoshapeoperator)     
    bpy.utils.register_class(shapetouvoperator)    
    
def unregister():
    bpy.utils.unregister_class(uvtoshapeoperator)  
